...
def train(dataset, architecture, path):
    ...
    val_every_epochs_num = 20         # Number of epochs between validation checkpoints
    ...
    elif architecture == 'regularized_hamiltonian':
        net = hamNet.HamiltonianRegularizedNetwork()
    else:
    ...
    for epoch in range(epoch_num):
        ...
        with tqdm(trainloader, unit="batch") as tepoch:
            ...
            if epoch % val_every_epochs_num == 0:
                # Validation step
                with torch.no_grad():
                    result = evaluate(net, valloader, criterion)  # Model is set to evaluate mode
...
def weight_smoothness_reg(net, weight_cache):
    ...
    Function that computes weight smoothness regularization and manages the weight cache.
    The regularization term is calculated by the following equation:
    Given a convolution weight matrix K, weight smoothness regularization is defined by:
    R_ws = sum(|K(j) - K(j+1)|^2)
    K(j) - Weights in previous state (before optimizer step).
    K(j+1) - Weights in current state.

    :param net: Model with weight to be regularized
    :param weight_cache: Memory to save weights in previous state (before optimizer step)
    :return: Updated weight cache and regularization term
    '''
...
if __name__ == "__main__":
    train(dataset='cifar10', architecture='regularized_hamiltonian', path='./hamiltonian_net3.pth')
    test(dataset='cifar10', architecture='regularized_hamiltonian', path='./hamiltonian_net3.pth')
